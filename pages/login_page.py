...
class LoginPage(BasePage):
    ...
    def register_new_user(self, email, password):
        ...
        password = "AsdFgh123$"
        user_email = self.browser.find_element(*LoginPageLocators.EMAIL_ADDRESS)
        user_email.send_keys(email)
        user_password = self.browser.find_element(*LoginPageLocators.PASSWORD1)
        user_password.send_keys(password)
        user_password_verify = self.browser.find_element(*LoginPageLocators.PASSWORD2)
        user_password_verify.send_keys(password)
        button_register = self.browser.find_element(*LoginPageLocators.BUTTON_REG)
